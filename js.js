/*Variavéis*/
let screenHome = document.querySelector('.screenHome');
let screenMain = document.querySelector('.screenMain');
let imgCkClose = document.querySelector('.imgCkClose');
let btnScreenMain = document.querySelector('button');
let phrases = document.querySelector('.phrases')

let randomNumber = () => {
    max = 70
    min = 1
    number = Math.round((Math.random() * (max - min) + 1))
    return number;
};
...
function openCookie () {
    ...
    screenHome.classList.remove('animationFadein');
    screenHome.classList.add('animationFadeout');

    screenMain.classList.remove('animationFadeout');
    screenMain.classList.add('animationFadein')
...
function returnHome () {
    ...
    screenMain.classList.remove('animationFadein');
    screenMain.classList.add('animationFadeout');

    screenHome.classList.remove('animationFadeout');
    screenHome.classList.add('animationFadein');

    setTimeout(() => {
        screenHome.classList.remove('hidden')
        screenMain.classList.add('hidden')
        phrases.innerText = phrasesCk[randomNumber()]
        console.log(number)
    }, 600);
};

function keyPress (event) {
    if(event.key == 'Enter' && screenHome.classList.contains('hidden')) {
        returnHome()
    } else {
        openCookie()
    }
...
/*Eventos */
imgCkClose.addEventListener('click', openCookie)
btnScreenMain.addEventListener('click', returnHome)
